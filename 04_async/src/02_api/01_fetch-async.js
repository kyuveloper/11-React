...
/* 
    ...
    url : 접근하고자 하는 url
    options : 선택 매개 변수로 http method나 headers, body 내용을 객체로 지정할 수 있다

    options에 아무 값도 넣지 않으면 기본 get 메소드 (8가지 http method)로 요청하게 된다

    fetch를 호출하면 브라우저는 네트워크 요청을 보내고 promise를 반환한다
    반환 받은 promise가 내장 클래스 response 인스턴스와 함께 이행 상태가 된다
...
async function callApi() {
    ...
    console.log(`응답 상태 : ${response.status}`); // 요청된 상태 코드(여기선 200) // 404도 응답 코드 중 하나
    console.log('응답 헤더');
    for (let [key, valye] of response.headers) { // 헤더 토큰..인증..
        console.log(`${key} : ${valye}`);
    }
    console.log(`본무 내용 사용 여부 : ${response.bodyUsed}`);
